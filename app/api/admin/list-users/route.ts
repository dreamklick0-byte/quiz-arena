...
export async function GET() {
  try {
    ...
    const diagnostics = {
      ...
      profilesCount: profilesData.data?.length || 0,
      hasProfilesData: !!profilesData.data,
      profileError: profilesData.error
    };

    console.log("Diagnostics:", diagnostics);

    // 2. Fetch supplementary data in parallel
    ...
    return NextResponse.json({ success: true, users, envCheck, diagnostics });
  } catch (err: any) {
