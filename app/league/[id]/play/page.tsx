 ...
 export default function LeaguePlayPage({ params }: { params: Promise<{ id: string }> }) { 
   ...
   const handleNext = useCallback(async () => { 
     if (index >= questions.length - 1) { 
       ...
       await supabase 
         .from("league_entries") 
         .update({ score, time_seconds: timeTaken, finished: true }) 
   ...
   }, [index, questions.length, score, startTime, leagueId, router]); 
         ...
         <div className="mt-12"> 
           ...
           <div className="mt-8 space-y-3"> 
             {q.options.map((opt: string, i: number) => { 
               ...
               return ( 
                 <button key={i} disabled={answered} onClick={() => pickOption(i)} 
