...
export default function SchoolPage() {
  ...
  const [mySchool, setMySchool] = useState<{ name: string; school_code: string } | null>(null);
  ...
  const generateSchoolCode = (name: string) => {
    ...
    const prefix = words.map((w: string) => w[0]).join("").slice(0, 4);
  ...
  const fetchData = useCallback(async () => {
    ...
    if (user) {
      setUserId(user.id);
      const { data: profile } = await supabase.from("profiles").select("school_id, school_code").eq("id", user.id).maybeSingle();
      if (profile?.school_id) {
        const { data: school } = await supabase.from("schools").select("name, school_code").eq("id", profile.school_id).maybeSingle();
        if (school) setMySchool(school);
      }
    }
    const { data } = await supabase.from("school_leaderboard").select("*").order("total_wins", { ascending: false }).limit(50);
  ...
  const handleRegister = async () => {
    if (!regName || !regEmail || !regState) { setMsg({ type: "error", text: "School name, email and state are required." }); return; }
    setBusy(true); setMsg(null);
    const supabase = getSupabaseClient();
    const code = generateSchoolCode(regName);
    const { data: school, error } = await supabase.from("schools").insert({ name: regName, email: regEmail, phone: regPhone, state: regState, city: regCity, address: regAddress, school_code: code }).select("id, name, school_code").single();
    if (error) { setMsg({ type: "error", text: error.message.includes("unique") ? "This email is already registered." : error.message }); setBusy(false); return; }
    if (userId && school) {
      await supabase.from("profiles").update({ school_id: school.id, school_code: school.school_code }).eq("id", userId);
      setMySchool({ name: school.name, school_code: school.school_code });
      setMsg({ type: "success", text: `School registered! Your school code is: ${code} — share this with your students.` });
      setTab("leaderboard");
      fetchData();
    }
    setBusy(false);
  ...
  const handleJoin = async () => {
    ...
    setBusy(true); setMsg(null);
    const supabase = getSupabaseClient();
    const { data: school, error } = await supabase.from("schools").select("id, name, school_code").eq("school_code", joinCode.toUpperCase().trim()).maybeSingle();
    if (error || !school) { setMsg({ type: "error", text: "School not found. Check the code and try again." }); setBusy(false); return; }
    if (userId) {
      await supabase.from("profiles").update({ school_id: school.id, school_code: school.school_code }).eq("id", userId);
      setMySchool({ name: school.name, school_code: school.school_code });
  ...
  return (
    <div className="min-h-screen bg-[#0f0f1a] text-zinc-100">
      <div className="py-14 text-center relative overflow-hidden" style={{ background: "linear-gradient(135deg, #0a1a2e, #1a0a2e)" }}>
      ...
      <div className="mx-auto max-w-lg px-4 py-6">
        <div className="flex gap-2 mb-6">
          {(["leaderboard", "register", "join"] as const).map((key) => (
            <button key={key} onClick={() => { setTab(key); setMsg(null); }}
              className={`flex-1 rounded-xl py-2 text-xs font-bold transition ${tab === key ? "bg-purple-600 text-white" : "bg-white/5 text-zinc-400 hover:bg-white/10"}`}>
              {key === "leaderboard" ? "🏆 Leaderboard" : key === "register" ? "🏫 Register" : "🔗 Join School"}
            </button>
          ))}
        </div>
          ...
          <div className="space-y-4">
            ...
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <button onClick={() => setTab("register")} className="mt-4 text-purple-400 text-sm font-bold hover:underline">Register your school →</button>
              </div>
            ) : (
                ...
                {filtered.map((school, i) => (
                  <div key={school.school_id} className={`rounded-2xl border p-4 ${i === 0 ? "border-yellow-500/40 bg-yellow-500/5" : i === 1 ? "border-zinc-400/30 bg-zinc-400/5" : i === 2 ? "border-orange-500/30 bg-orange-500/5" : "border-white/10 bg-[#161627]/60"}`}>
                    <div className="flex items-center gap-3">
                      <span className={`flex h-9 w-9 items-center justify-center rounded-xl font-black text-sm ${i === 0 ? "bg-yellow-500 text-black" : i === 1 ? "bg-zinc-300 text-black" : i === 2 ? "bg-orange-500 text-black" : "bg-white/5 text-purple-400"}`}>{i + 1}</span>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-white/10 bg-[#161627]/60 p-5 space-y-4">
              ...
              <div>
                ...
                <input value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())} placeholder="e.g. GCL1234"
            ...
            <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/5 p-4 text-center">
              <p className="text-xs text-zinc-400">Don't have a code? <button onClick={() => setTab("register")} className="text-yellow-500 font-bold hover:underline">Register your school</button>.</p>
            </div>
          </div>
        )}
